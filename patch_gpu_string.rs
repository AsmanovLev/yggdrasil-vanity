...
pub fn main() {
    ...
    // run_opencl needs to parse args.device_idx which is now a String
    // I need to change the run_opencl logic entirely to handle String.
    
    // I will replace run_opencl with a version that works with Args having String
    // I need to do this carefully because I replaced the entire function previously.
    
    // This is getting too complex for sed/replace. 
    // I will re-write run_opencl from a scratch variable.
...
fn run_gpu_instance(args: &Args, device_idx: usize, best_score: Arc<AtomicI64>, cand_tx: mpsc::SyncSender<(Vec<u8>, usize)>, app_start: std::time::Instant) {
    ...
    let mut gpu = Gpu::new(GpuOptions { platform_idx: args.platform_idx, device_idx: device_idx, threads: args.threads, local_work_size: args.local_work_size }, mode, args.weight_h, args.weight_l, args.minimal_height).unwrap();
